...
export default function EditEventDialog({ event }: EditEventDialogProps) {
    ...
    useEffect(() => {
        const loadEventData = async () => {
            if (event) {
                try {
                    const res = await fetchAPI(`/events/${event.event_id}`);
                    const eventData = res.data;
                    setEventName(eventData.event_name);
                    setDateRange({
                        from: new Date(eventData.start_date),
                        to: new Date(eventData.end_date),
                    });

                    if (eventData.prizes.length > 0) {
                        const updatedPrizes = await Promise.all(
                            eventData.prizes.map(async (prize: any, index: number) => {
                                const imageFile = prize.image
                                    ? await urlToFile(
                                          `${process.env.NEXT_PUBLIC_API_BASE_URL}/${prize.image}`,
                                          `prize-${index + 1}.png`,
                                          'image/png'
                                      )
                                    : null;
                    ...
                    showAlert("error", "An error occurred while fetching the event data.");
                    setLoading(false);
                }
            }
        };
    ...
    const handleSave = async () => {
        ...
        try {
            await fetchAPI(`/events/${event.event_id}`, {
                method: 'PUT',
                body: formData,
            });
